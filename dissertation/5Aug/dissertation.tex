...
\textbf{Introduction protocol.} Participants are shown each visualisation with only a brief description of the project's purpose (``We are studying how literary place names can be visualised''), but no explanation of what each visual encoding means. This ``cold'' presentation matters methodologically, since it tests whether the designs are self-explanatory rather than testing participants' ability to apply an explanation they were given: a participant who needs the design explained before they can use it is, in effect, reporting a usability problem.
 
\textbf{Tasks.} Two task types are used. Task~1 (fingerprint direction): participants are shown a fingerprint visualisation (radar chart, bar-code, or small multiples) and asked which of the five authors they believe it represents. This is a forced-choice identification task; accuracy is the primary metric. Because the six canonical prototypes described in Chapter~\ref{chap:design} render all five authors together, with each identified by name in a legend, row label, or panel header, they cannot serve as the stimulus for a blind identification task without simply giving the answer away. Task~1 therefore uses a purpose-built single-author variant of each of the three fingerprint designs, showing one author's shape only, with the legend and the click-through detail panel (which would otherwise expose book titles and could reveal identity through bibliographic or authorial-voice recognition) removed entirely; only the shape and its hover-revealed sector values remain. A different author is used for each of the three designs, to avoid a participant's answer to the first fingerprint task anchoring their answers to the second and third. Task~2 (topology direction): participants are shown a topology visualisation (force-directed network, linear connection diagram, or metro map) and asked to identify the two places that appear most strongly connected, and to judge whether those two places are geographically close or far apart in Edinburgh. These three designs already aggregate all five authors' co-occurrence data into one structure, so no equivalent blinding is needed. The secondary question tests whether the visualisation spontaneously communicates the narrative/geographic divergence finding. After completing all tasks, participants vote for the most intuitive design in each direction.
...
\textbf{Task 1 --- fingerprint identification (shown once per fingerprint design: radar, bar-code, small multiples).}
\begin{enumerate}
  \item You are looking at a visualisation of one author's use of Edinburgh place names in their writing. Which of the following five authors do you think it represents? \emph{(Alexander McCall Smith / Irvine Welsh / John Gibson Lockhart / Walter Scott / Robert Louis Stevenson / I don't know)}
  \item How confident are you in this answer? \emph{(Not at all confident -- Very confident, 5-point scale)}
  \item In one or two sentences, what about the visualisation led you to that answer?
