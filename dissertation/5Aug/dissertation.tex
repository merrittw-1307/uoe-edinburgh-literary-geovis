...
% 等第7周问卷做完之后，再重新排一下这部分和问卷相关章节的优先级。整合界面除了前面那次性能/可读性调整之外，现在还经过了一次导师指导下的界面体验修改；除非问卷结果或者下次开会又提出新需求，这部分先不用再动了。
...
\end{enumerate}
A single forced-choice identification (item 1) only ever yields a correct/incorrect judgement against a one-in-five baseline; the runner-up question (item 2) is added cheaply, from the same stimulus, to distinguish participants who narrowed the shape down to a plausible pair from those guessing entirely at random, since both would otherwise register identically as ``incorrect.''
...
\begin{enumerate}
  ...
  \item If no one had told you anything about this visualisation, would you have understood how to read it just by looking? \emph{(Yes, easily / Yes, with a little effort / No, I was confused)}
\end{enumerate}
Items 3 and 6 are added for the same reason as the fingerprint task's item 8 above: item~1 alone risks a ceiling effect if the strongest pair in a given dataset is visually unmistakable (here, Leith and Princes Street, at a co-occurrence weight of 28 against a next-highest weight of 18), and item~5 alone risks a floor effect if participants who notice nothing simply skip it, yielding no usable data either way. Item~3 asks participants to rate three place pairs deliberately chosen to span strong, medium, and weak ground-truth co-occurrence weight (New Town--Princes Street at 18, Lochend--Waverley Station at 4, and Leith--Silvermills at 2, avoiding the single strongest pair already targeted by item~1) rather than search for a pair themselves, producing interval-scaled data that can be correlated against the real weights regardless of whether item~1 saturates. Item~6 gives a forced choice between one real community-detected cluster and two distractors built by mixing places from different clusters, providing a scoreable recognition-based measure to sit alongside item~5's open-ended production task; agreement between what a participant names unprompted in item~5 and what they select in item~6 is treated as corroborating evidence in the analysis. For the metro map, the true cluster in item~6 is drawn from that design's own author-named lines (the stations of ``Lockhart's Edinburgh''); for the force-directed network and linear diagram, which share identical underlying edge weights and have no native line structure, the true cluster is instead one of the communities returned by Louvain community detection on the same co-occurrence graph, and items~3 and 6 accordingly use identical wording and place names across those two designs.
