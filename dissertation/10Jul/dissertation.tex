...
A framework scaffold for the combined interface has been built (\texttt{data/processed/combined/d3/combined\_interface.html}), implementing the navigation shell described in the original plan while deferring the live-recompute logic to a later iteration. The shell provides top-level navigation between the two directions (Fingerprints / Topology), a left-hand panel that switches between the three visualisations within the active direction, a main view, and a right-hand detail panel, matching the intended layout. Rather than reimplementing each visualisation's rendering logic a second time inside a shared page, the six canonical HTML files (Section~\ref{sec:technical-decisions}) are embedded unmodified via \texttt{<iframe>}, one per visualisation, with only the active one made visible; this reuses every existing interaction (hover tooltips, click-through detail panels, zoom/pan, search) exactly as tested individually, at the cost of the six visualisations currently running as independent documents rather than sharing state.

Two features from the original plan are scaffolded but not yet functional, and are marked as such in the interface itself (a dashed border and a ``planned'' tag): the author selector and per-visualisation parameter sliders (weight threshold, sort order), and cross-view linking (selecting an author in one direction highlighting their places in the other). Both are deferred rather than implemented with placeholder behaviour, since a non-functional-looking control is less misleading than one that appears to do something but does not. The scale-exploration datasets built for Appendix~\ref{app:scale} (\texttt{all\_authors\_radar.json}, \texttt{all\_authors\_network.json}, \texttt{all\_authors\_barcode.json}, \texttt{all\_authors\_small\_multiples.json}) already contain live-recomputable top-place and co-occurrence data for any author subset up to all 424 authors, and are the intended data source for the author selector once implemented: at that point the six \texttt{<iframe>}s would be replaced by six render functions reading from a single shared \texttt{selectedAuthors} array, with the surrounding shell (navigation, panel layout) unchanged. The metro map is the one exception, since its lines depend on re-running community detection offline (Section~\ref{sec:metro-redesign}) rather than a client-side recomputation; the plan is for it to switch between a small number of precomputed snapshots (as already produced for 5, 20, and up to 50 authors in the scale exploration) rather than recompute per selection.

% TODO: revisit priority against the user-study-dependent chapters once the study itself has run in Week 7; full cross-view linking and the live author selector are the main remaining implementation work if time allows.
