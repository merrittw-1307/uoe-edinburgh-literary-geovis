...
## 16c. 同日会话再追加：outer detail面板接入api_sentence（Merritt要求"detail面板也接进去，用api_sentence"）

Merritt紧接着要求把上一轮标注为"锦上添花、暂未做"的outer detail面板也真正接上，数据来源明确要求是`api_sentence`表。

### 新增数据：`generate_all_authors_sentences.py`

新脚本几乎完全照抄`dir_1/radar/py/generate_dir1_sentences.py`的采样逻辑（同样的随机种子42、每对最多3句、220字截断），**唯一的改动是去掉WHERE作者过滤条件**，改成对全部408位作者、每人只取自己mention数最高的top15地名（跟radar/barcode/network/linear三个live视图已经在用的"top15"惯例完全对齐——detail面板只需要覆盖UI上实际能hover到的(author, place)组合，不多算）。查询结果：62,262条带句子的mention记录里，限定到top15后剩4,589对(author, place)组合、42,012行，最终产出`all_authors_sentences.json`（2.4MB，408位作者全覆盖）。

### 接入方式：新增第五份数据集，不复用旧的5作者专属数据

`network_enriched.json`（正式版network.html用的）已经有一套`placeSentences`/`placeBooks`/`edgeBooks`结构，但那是锁定5作者算好的，无法覆盖作者选择器允许的任意子集（2/5/20/50或搜索添加）。所以没有复用它，而是新建了按`(author, place)`索引的数据集，在detail面板渲染时对"当前选中的所有作者"逐一查表、合并去重——这样无论选中哪个子集，detail面板都能查到对应作者在这个地名下的句子，书目列表则是从已经嵌入的network mentions数据里实时按`place`+`selectedAuthors`过滤聚合出来的（不需要额外数据集，跟已有的`MENTIONS`数组共享）。

### 触发方式

radar（点击/hover多边形）、barcode（hover柱子）、small_multiples（hover地图标记）、network（hover节点）、linear（hover节点）五个视图的现有hover事件里都追加了一行调用`showDetailForPlace(place, authors)`（radar比较特殊，vertex是sector不是地名，所以用`showDetailForAuthor(name)`取该作者mention最高的地名作为代表样例）。metro保持iframe，**不接入**detail面板——跟上一轮的cross-view focus一样，如实在界面上写明这个限制，不假装metro也联动。

面板设计成"粘性"：hover移开后内容不会立刻清空，方便读完句子；只有切换可视化类型或作者选择变化时才清空（避免残留一条指向已经不在视图里的地名/作者的旧句子）。

### 浏览器测试（实际DOM事件验证，不是直接调函数）

用`dispatchEvent(new MouseEvent('mouseover', ...))`真实触发DOM事件（不是直接调用JS函数走后门），确认五个视图的事件监听器都正确接上：
- network hover任意节点 → detail面板显示地名、sector、书目列表（按mention数排序）、最多2句真实例句+书名+作者
- barcode hover柱子 → 正确显示该地名信息
- small_multiples hover地图圆点 → 正确显示该地名信息
- radar hover多边形 → 正确显示该作者mention最高的地名（如Alexander McCall Smith → "Leith"）
- 切换视图后面板正确清空回到提示文案，控制台全程无报错

### 论文改动

`\section{Combined Interface}`第三次改写：把"outer detail面板未实现，是锦上添花"这句删掉，换成完整描述——新数据集的生成方式（复用已验证的采样方法，只去掉作者过滤+限定top15）、查表+合并的渲染逻辑、radar用"最高频地名代表"处理sector粒度不匹配的问题、面板的粘性/清空设计理由、metro不参与的说明。
...
## 18. 同日追加：实际打开浏览器复测大规模作者池，修了3个真bug

Merritt追问"扩大作者池后效果有问题吗"，逼着我别光靠记忆/文字检查，去真的点开浏览器测。结果测出3个之前从没被肉眼验证过的真问题：

1. **Radar颜色算法缺陷**：`(idx*47)%360`不是均匀分布，50个作者里有两人色相只差1度，肉眼分不清——改成`idx*360/total`保证最小色差。
2. **左侧作者chip列表没限高**：50人时把侧边栏撑到1425px，"Cross-view Focus"要滚很远才能看到——加了140px滚动框。
3. **条形码的柱子高度恒为0px，完全不可见**——这个最严重，而且不是新bug：`.bar-wrap`没设显式高度，导致子元素`.bar`的百分比高度没有可参照的容器高度，CSS规范下直接坍缩成0。**这个bug从scale-exploration的barcode工具最早写出来那次就存在**（Task #13"完成"时只检查了行数和渲染耗时，从没实际看过柱子是否显示），Combined Interface的barcode视图原样复制了这份CSS，所以也一样坏。两处都修了（`.bar-wrap`加`height:60px; justify-content:flex-end`）。canonical版`barcode.html`用SVG画柱子，不受影响。论文里对barcode的规模发现是纯文字/耗时数据，没有截图，所以这个bug不影响已写进论文的结论。

**教训**：之前测试barcode时只检查了"行数对不对""渲染要多久"，没有真正确认柱子渲染出来了——结构对不代表视觉对。以后测图表类的东西，要检查关键视觉元素（柱子/线条/点）的computed height/width是不是非零，不能只看DOM结构。

---

## 19. 同日追加：metro图"参数不变化"问题（Merritt看完地铁图评估后追问）

Merritt问完地铁图"解释性怎么样"之后，实测发现2和5作者、20和50作者显示的图完全一样——因为Combined Interface之前只有两份预计算快照（5作者canonical + 20作者），用一个简单阈值（≤10 vs >10）二选一，20和50都落在">10"这一档，2和5都落在"≤10"这一档，所以看起来"参数不生效"。

**做法**：补算了2作者（96%书目支撑）和50作者（73%）两份完整快照（之前只有20作者算过完整版，2/50只有汇总统计），另外给独立的scale-exploration metro工具也补算了一份独立算的5作者版本（跟canonical版本数值有细微差异，原因NOTES.md已经记录过，不是bug）。Combined Interface改成从2/5/20/50四档里按对数距离选最接近的，四个预设按钮现在都对应不同的真实结果。同时把独立的metro scale-exploration工具从"只有一个固定20作者的文件"重做成跟其他五个工具一样、带预设按钮可切换的`metro_scale_explore.html`。

**另外把"20+作者看起来乱"这件事的提示做得更醒目**：原来只是一行小字，容易被忽略以为是bug；现在20/50作者时会弹出醒目的红色警告条，明确说"这张图在演示社群检测算法本身在这个规模下会失效，不是一个可用的探索工具，看设计意图请用2或5"——把"失效案例"和"探索工具"这两种不同的东西在界面上区分开。
