...
\abstract{
% DRAFT v1 -- sentences 1-8 are not study-dependent and should be close to
% final. The last 2-3 sentences summarise the corpus-analysis findings that
% do not need the study (RQ2); a sentence reporting the RQ1 identification
% accuracy (Chapter 5) still needs to be added once the study has run.
Existing digital tools for literary geography plot place names as points on a map, treating them as geographic data; but the meaning a place name carries in a literary text comes from its narrative context, not its coordinates. This dissertation asks whether that narrative structure can be made visible directly, using the LitLong Edinburgh database of 620 literary works (1687--2015), 2,135 place names, and 50,248 place-name mention records. Two research questions are addressed. RQ1 asks whether an author's distribution of place-name mentions across the city forms a visually distinctive spatial ``fingerprint.'' RQ2 asks whether the co-occurrence of place names within literary works reveals a narrative topology distinct from their geographic topology. Six interactive D3.js visualisation prototypes were designed and implemented across these two directions --- a radar chart, a bar-code fingerprint, and small-multiple maps for RQ1; a force-directed network, a linear connection diagram, and a metro-style map for RQ2 --- and evaluated through a user study with domain-expert and general-public participants. The corpus analysis underlying RQ2 shows that Leith and Princes Street, roughly 2.5km apart, are the most strongly co-occurring place pair in the corpus (28 shared works); rebuilding the metro-style map's line structure from measured co-occurrence rather than hand-drawn geography raises the proportion of adjacent stations sharing a real book from 22\% to 88\%, and surfaces further instances of narrative structure that geographic proximity alone could not reveal. % TODO: add RQ1 finding once the user study has run (identification accuracy for the fingerprint designs).
}
...
\caption{The same map after the redesign: five lines derived from modularity clustering on the co-occurrence graph and named after their dominant author, with 88\% of adjacent station pairs now backed by measured co-occurrence. Compare against Figure~\ref{fig:metro-before} --- the line structure itself, not just the layout, has changed.}
...
Five of the six visualisations (radar, bar-code, small multiples, network, linear) render live in-page from a single shared \texttt{selectedAuthors} array, reusing the same client-side top-15-place and document-level co-occurrence computation already built and validated for the scale exploration (Appendix~\ref{app:scale}): the preset buttons (2 / 5 / 20 / 50 authors) and free-text search operate on the same four JSON bundles (\texttt{all\_authors\_radar.json}, \texttt{all\_authors\_barcode.json}, \texttt{all\_authors\_small\_multiples.json}, \texttt{all\_authors\_network.json}) rather than duplicating a new data pipeline. Cross-view linking is implemented as a single \texttt{focusedAuthor} value alongside the selection: clicking an author's polygon (radar), row (bar-code), or map panel (small multiples) toggles focus and re-renders that view with the focused author emphasised and the rest dimmed; switching to a Topology view recomputes that author's own top-15 places and highlights the matching nodes/edges gold within the current selection's combined graph, dimming the rest. This directly implements the planned behaviour (``selecting an author in fingerprint view highlights their places in topology view'') without requiring \texttt{postMessage} between iframes, since the five views are now rendered in the same document rather than embedded separately.

The metro map remains the one exception: because its lines depend on offline community detection (Section~\ref{sec:metro-redesign}) rather than a computation that can run per keystroke in the browser, it stays an \texttt{<iframe>} that switches between four precomputed snapshots (2, 5, 20, and 50 authors, chosen by nearest log-distance match to the current selection) rather than recomputing live, and does not participate in the focus highlight or the detail panel below. An earlier version of this switch only had two snapshots and picked between them with a fixed threshold, so selecting 2 versus 5 authors, or 20 versus 50, produced no visible change at all; three snapshots have since been added at 2 and 50 authors (96\% and 73\% adjacency-backed respectively, extending the monotonic trend already reported for 5 and 20) so every preset now shows a genuinely different, correctly rendered map. Past roughly 20 authors the snapshot itself is showing community detection degrade, not a usable exploration surface, so both the standalone scale-exploration tool and the combined interface display a prominent warning at that scale rather than a quiet note, making clear this is a reported finding and not a rendering fault.

A unified outer detail panel, shared across the five live views, is also implemented. It is backed by a fifth dataset, \texttt{all\_authors\_sentences.json}, generated by a new query against \texttt{api\_sentence} that mirrors the query already used for the five-author prototypes (same sampling, same 220-character truncation, same random seed) but drops the author filter and restricts each of the 408 authors to their own top-15 places by mention count -- exactly the set reachable through the live selector, so no sentence data is generated for places a user could never hover over. Hovering a place in bar-code, small multiples, network, or linear looks up every author in the current selection against this dataset and merges their example sentences and mention-weighted book list for that place; hovering an author's shape in the radar chart shows their single most-mentioned place as a representative example, since radar's own vertices are sectors rather than individual places. The panel is deliberately sticky (it does not clear on mouse-out) so a sentence can be read after the cursor moves away, and it does clear when the active visualisation or the author selection changes, since a stale sentence would otherwise reference a place or author no longer in view.

Selecting a large author subset in network or linear was found to produce enough co-occurrence edges (12,833 at the 50-author preset) that d3's force simulation failed to settle within several seconds, leaving the interface visibly unresponsive; the weight-threshold slider was also found to be capped at a fixed maximum of 10 regardless of the true maximum edge weight in the current selection (which reaches into the 80s at scale), so it could not be used to filter down far enough to recover. Both views now cap rendering to the 1,500 strongest-weighted edges with the slider's maximum computed dynamically from the current graph, and the underlying author selection is left untouched by the cap so switching to another view still reflects the full selection. A related legibility issue was found in the radar chart's per-author colour assignment: a fixed-step hue formula placed two of fifty authors' colours one degree apart on the colour wheel, indistinguishable despite being on the same chart; hues are now spaced evenly across the current selection size, guaranteeing a minimum perceptual gap between every pair shown.

A supervision meeting reviewing the interface at this stage produced three further revisions. First, the interface had accumulated implementation-facing language visible to anyone using it -- the top banner named \texttt{api\_sentence} and internal rendering limits, and small badges read ``live''/``implemented'' next to section headings -- none of which helps someone exploring the data decide what to do next; this was rewritten throughout in plain terms (for example, the metro map's note now reads that it is ``pre-drawn rather than redrawn live'' instead of describing offline community detection). Second, the radar chart's hover interaction was found to only offer one tooltip per author's entire polygon, giving no way to inspect a single sector; a small marker was added at each of the 14 sector vertices, and hovering one now shows that author's mention count and percentage for that specific sector together with the individual place names contributing to it (cross-referenced from the bar-code dataset), updating the outer detail panel to that sector's leading place. The small multiples map markers were extended similarly, gaining sector and percentage-of-total alongside their existing place and mention count. Third, author selection previously offered only fixed presets plus one-at-a-time typed search, which does not make ``pick any combination of the 408 authors'' obvious as a supported action; a collapsible, scrollable checklist of all 408 authors (sorted by mention count) was added alongside the existing controls, and a one-line note now clarifies that the ``2''/``5'' presets are the project's fixed core-author set while ``Top 20''/``Top 50'' means ranked by total corpus mentions, rather than leaving ``top'' unexplained.
...
A fifth author preset, loading all 408 located authors at once, was added alongside the existing 2/5/20/50 presets and explicitly labelled ``large-scale exploration'' in both its button text and an accompanying note, distinguishing it from the four practical presets: it is intended for spotting broad, corpus-wide patterns rather than reading individual detail, and the rendering caps already described above for network, linear, small multiples, and metro are what keep it usable rather than something to disable at this scale.

% TODO: revisit priority against the user-study-dependent chapters once the study itself has run in Week 7. The combined interface has now been through one supervised UX review in addition to the performance/legibility pass above; no further work is scoped unless the study or a further supervision meeting surfaces a need.
...
\textbf{Five-author prototype, tested at scale.} The current system uses five test authors, selected for contrast and representativeness. Whether this scales to the full corpus was tested directly rather than left as an open question, for all six designs, using a set of scale-exploration tools kept separate from the six canonical five-author prototypes (radar, network, and linear are live interactive pages that recompute their data client-side for any of the 408 authors with location data; bar-code is likewise live; small multiples and metro required batch or capped-live testing for reasons specific to each design, detailed below). The headline result is that the six designs do not share one scaling bottleneck: each fails in a different, specific way, summarised in Table~\ref{tab:scale-summary} (Appendix~\ref{app:scale}).
...
\textbf{RQ2 --- does narrative co-occurrence reveal a topology distinct from geography?} Yes. The strongest evidence is the Leith/Princes Street pair itself: 28 shared works despite roughly 2.5km of geographic separation, and one of only 12 station-pairs across the entire redesigned metro map that behave as a genuine narrative interchange. The redesign of the metro-style map (Section~\ref{sec:metro-redesign}) turned this from a single anecdote into a systematic result: deriving line structure from measured co-occurrence rather than hand-drawn geography raised the proportion of adjacent stations sharing a real book from 22\% to 88\%, and surfaced two further instances of the same divergence that geography could not have produced --- the word ``Leith'' clustering with Scott and Lockhart's historical corridor rather than Welsh's, and a biographical corridor (Castle Street, Lasswade, the University of Edinburgh) distinct from the historical-fiction corridor immediately next to it. The scale-exploration work (Appendix~\ref{app:scale}) adds a boundary condition rather than a contradiction: narrative and geographic topology diverge clearly at the five-author scale used throughout this dissertation, but the mechanism generating that divergence --- near-total non-overlap of authorial vocabularies (Finding~1) --- means the co-occurrence signal itself thins out at very large author counts and degenerates at very small ones (2--3 books), so the divergence is most legible at a middle scale, not a universal property independent of $N$.

% TODO: fill in after user study
...
Several directions for future work are identified. The combined interface (Section~\ref{sec:combined-interface}) already provides author-selection controls reaching the full 408-author corpus for five of the six views, with rendering safety caps keeping it responsive at scale; a remaining gap is the metro map's dependence on offline community detection, which currently limits it to a small number of precomputed author-count snapshots rather than an arbitrary live selection -- a faster, approximate community-detection method that could run client-side would close this. The interactive detail panels are likewise already grounded in \texttt{api\_sentence} source text (Section~\ref{sec:combined-interface}); a remaining extension would be a single outer panel shared consistently across all six canonical views, rather than each view's own panel plus the combined interface's separate one. The \texttt{api\_posmention} table, containing over two million part-of-speech annotations, would support a further research direction: narrative weight analysis, examining whether a place name appears in dialogue, narration, or description, and whether this varies systematically across authors or genres. Finally, the concept of ``literary silences''---Edinburgh places that appear in the gazetteer but never in any literary work---would complement the current analysis by revealing what Edinburgh's literary imagination systematically excludes.
