...
\textbf{Finding 1: Author spatial languages are highly individual.} Among the five test authors selected for prototyping (Alexander McCall Smith, Irvine Welsh, John Gibson Lockhart, Walter Scott, and Robert Louis Stevenson), no single place name is mentioned by three or more authors, and only 18 place names are shared by any two. That near-total absence of overlap confirms the literary-geographic intuition that each author constructs a largely private imaginary Edinburgh, with a direct implication for visualisation design: individual place names cannot serve as comparable axes across authors, because most axes would be zero for most authors. It was this finding that motivated the decision to use city sectors as the radar chart axes instead.
 
\textbf{Finding 2: Sector distributions cross-validate against literary knowledge.} The sector-based analysis yields distributions that align with established literary-geographic knowledge for all five test authors: McCall Smith concentrates in New Town (43.6\%), consistent with his Scotland Street series; Welsh concentrates in Leith (44.3\%), consistent with \textit{Trainspotting}; Scott concentrates in Old Town (32.0\%), consistent with his historical fiction; and Stevenson shows a notable Pentlands component (17.0\%), consistent with his documented connection to Swanston. Between them these give reasonable confidence in the sector classification methodology.
...
The primary interaction is point-level: hovering over any vertex reveals the sector percentage and a ranked list of the places and books that contribute to it, directly addressing the supervisor's feedback that the visualisation should provide ``low-level information upon interaction.'' A detail panel opens on click, listing the top place names and top books in the selected sector for the selected author; hovering over an individual place row within the panel additionally surfaces a real example sentence drawn from \texttt{api\_sentence}, together with the book and author it is drawn from. The legend supports filtering by author (click to hide/show) and isolation (double-click to show only one author), enabling the user to compare any pair or subset of authors.
...
The Y-axis scaling went through three iterations before reaching its final form. The first prototype used a logarithmic scale, following standard practice for long-tail frequency distributions, but supervisor feedback noted that logarithmic scales are difficult for non-specialist users to interpret intuitively, and that the shape of the distribution mattered more here than its absolute magnitude. The second prototype switched to a linear scale shared across all five authors, which ran into a different problem: Welsh's Leith alone accounts for 44.3\% of his total mentions, substantially more than any single place for any other author, so on a shared axis scaled to that maximum, every other author's distribution was compressed into a band occupying less than a quarter of the chart height, and within-author variation became imperceptible. The final design resolves this with a per-author independent Y-axis, where each row is scaled to its own maximum, prioritising the visibility of within-author spatial structure (the \emph{shape} of each author's distribution) over cross-author magnitude comparison. The trade-off, that bar heights can no longer be compared directly across rows, is made explicit by a warning note in the interface; a shared-axis mode scaled to the global maximum remains available via a toggle for when cross-author comparison is what's wanted.
...
\textbf{Force-directed network (primary design).} The force-directed network renders the document-level co-occurrence structure as a node-link diagram. Each node represents a place name; node size encodes total mention frequency across all authors. Each edge represents a co-occurrence pair, with edge width encoding co-occurrence weight using the function $w' = (w / w_{\max})^2 \times 12$, reached after testing two earlier mappings. A linear mapping ($w' = w \times 0.3$) produced edges that were visually indistinguishable in width across the full weight range 1--28; a square-root mapping ($w' = \sqrt{w/w_{\max}} \times 8$) improved discrimination marginally but still left the strongest connections insufficiently prominent relative to weaker ones. The squared function finally used produces a clear visual hierarchy while still rendering weaker connections as visible, thin lines.
...
\textbf{Metro-style map (illustrative design).} The metro-style map arranges place names as stations on a small number of named lines, following the visual language of the London Underground map. The London Tube map, designed by Harry Beck in 1933, replaced geographic accuracy with functional topology: it told travellers which line to take and where to change, rather than where stations were physically located. This project applies the same substitution to literary geography, with lines representing narrative corridors (clusters of places that co-occur strongly in the same works) rather than physical routes. Beck's map, however, still encoded a real network: the lines corresponded to the physical tracks trains actually ran on, and only the geometry was distorted for legibility. An initial prototype of the literary metro map lacked that property. Eight lines were defined manually along geographic and cultural axes (a northern coastal line, an Old Town line along the Royal Mile, and so on), and station order within each line followed geographic sequence rather than measured narrative connection.
...
This pipeline reduces the map to five data-derived lines (rather than eight hand-drawn ones) and raises the proportion of adjacency backed by real co-occurrence from 22\% to 88\% (46 of 52 adjacent station pairs; per-line figures are reported in Appendix~\ref{app:metro}). The community structure also surfaced two findings invisible in the hand-drawn version, discussed further in Section~\ref{chap:discussion}: the word ``Leith'' clusters with the historical Old Town corridor associated with Scott and Lockhart rather than with the Welsh-dominated periphery line, and a small five-station line (Castle Street, Lasswade, the University of Edinburgh, the Royal Society of Edinburgh, and St Giles) emerged as a coherent biographical corridor, corresponding to the places Lockhart's \textit{Life of Walter Scott} associates with Scott's own residences and professional life rather than with Scott's fiction.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_metro_v1_geographic.png}
\caption{The initial hand-drawn metro-map prototype, before the redesign. Lines follow geographic and cultural axes (a North Line, an Old Town Line, an East-West Line) chosen by inspection, with only 22\% of adjacent station pairs backed by measured co-occurrence.}
\label{fig:metro-before}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_metro.png}
\caption{The same map after the redesign: five lines derived from modularity clustering on the co-occurrence graph and named after their dominant author, with 88\% of adjacent station pairs now backed by measured co-occurrence. Compared with Figure~\ref{fig:metro-before}, it is the line structure itself that has changed here, not just the layout.}
...
A second pass over the same five-author mention data joins \texttt{api\_sentence} and \texttt{api\_document} to attach, for every place, a small sample of full source sentences (deduplicated, truncated to 220 characters, with the contributing book and author) and a ranked list of books that mention it. For Direction~2, the same join is additionally computed per co-occurrence edge, so the pair of places behind an arc or a network edge can be traced back to the specific books in which both appear. None of this alters any previously computed percentage, count, or co-occurrence weight; it is a separate lookup table, keyed by place name (and by place pair for edges), embedded alongside the existing data in each HTML file. Because sampling from \texttt{api\_sentence} surfaces the source text verbatim, it can include informal register and strong language where the original does (for example, in Irvine Welsh's Scots-dialect prose). This is an authentic property of the corpus rather than a sampling artefact, but worth flagging before using these visualisations in a supervisor demonstration or a public-facing study session, since a hover tooltip is not the reader's first encounter with the surrounding literary context.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_radar.png}
...
Hover interaction is implemented using invisible \texttt{circle} elements of radius 14 pixels centred on each vertex, a value chosen after testing smaller ones: 6 pixels requires pixel-precise cursor placement and is difficult to target reliably, whereas at 14 pixels hovering is comfortable without the targets overlapping each other. On hover, a tooltip shows the sector name and percentage; on click, a detail panel opens showing the top eight place names in that sector for the selected author, ordered by mention count, together with a bar chart and the top five books.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_barcode.png}
...
Sector boundaries are visualised using two complementary encodings: a low-opacity background rectangle in the sector's colour spans the bars belonging to each sector, and a narrow colour strip below the bar area identifies the sector. Text labels are rendered below the colour strip only when the sector is wide enough to accommodate them without overlap (threshold: 40 pixels), with narrower sectors identified by colour alone, which avoids the text-stacking problem earlier versions had when all 14 sector labels were displayed regardless of available width.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_small_multiples.png}
...
Each of the five panels is implemented as an independent Leaflet.js map instance, initialised with the CartoDB Positron tile layer. Place-name bubbles are rendered as Leaflet \texttt{CircleMarker} objects, whose \texttt{radius} property is recomputed whenever the scale mode changes. Earlier prototypes used a two-stage Leaflet/D3 overlay, positioning D3 SVG elements over the map via screen-coordinate projection; this was replaced with native Leaflet \texttt{CircleMarker} objects, which handle map pan and zoom correctly without any manual coordinate projection.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_network.png}
...
Node labels are positioned at $y = \text{node.y} + r + 12$ pixels, placing them below each node. An earlier version positioned labels at the node centroid with white fill, on the assumption the text would appear inside the node; in practice the labels were invisible, since the white text rendered against the equally dark node background. Moving labels below the node boundary fixed this.
 
\subsection*{Linear connection diagram}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_linear.png}
...
The redesigned pipeline described in Section~\ref{sec:metro-redesign} is implemented as two Python scripts. \texttt{build\_metro\_lines.py} performs the community detection, station sequencing, interchange detection, and layout, and writes a single JSON file containing, per line, its name, colour, member stations, each station's shared $(x,y)$ grid coordinate, and the adjacency-backed diagnostic count. \texttt{build\_metro\_html.py} then consumes this JSON and renders the final HTML file, embedding the data as a JavaScript object literal in the same manner as the other five visualisations. Keeping data derivation separate from rendering means the underlying clustering parameters (community-detection threshold, interchange threshold, maximum line size) can be adjusted and the map regenerated without touching the rendering code at all.
...
\textbf{Colour accessibility.} The five fixed per-author colours were checked against simulated protanopia, deuteranopia, and tritanopia (the Brettel/Vi{\'e}not transform applied to each hex value) rather than assumed to be distinguishable. All ten pairs remain visually separable under all three simulations; the closest pair in every case is McCall Smith and Scott, noticeably closer together than the other pairs though still above the threshold treated here as a likely confusion risk. This is a reasonable but not maximally optimised palette, since the colours were originally chosen for contrast against the interface background rather than for colour-vision-deficient legibility specifically.
 
\section{Combined Interface}
\label{sec:combined-interface}

The combined interface (\texttt{data/processed/combined/d3/combined\_interface.html}) implements the navigation shell described in the original plan and, unlike the first iteration, the author selector and cross-view linking are now functional rather than scaffolded. The shell provides top-level navigation between the two directions (Fingerprints / Topology), a left-hand panel switching between the three visualisations within the active direction plus the shared author selector, a main view, and a right-hand detail panel.

Five of the six visualisations (radar, bar-code, small multiples, network, linear) render live in-page from a single shared \texttt{selectedAuthors} array, reusing the same client-side top-15-place and document-level co-occurrence computation already built and validated for the scale exploration (Appendix~\ref{app:scale}). The preset buttons (2 / 5 / 20 / 50 authors) and free-text search operate on the same four JSON bundles (\texttt{all\_authors\_radar.json}, \texttt{all\_authors\_barcode.json}, \texttt{all\_authors\_small\_multiples.json}, \texttt{all\_authors\_network.json}), so no new data pipeline had to be built just for this. Cross-view linking is implemented as a single \texttt{focusedAuthor} value alongside the selection: clicking an author's polygon (radar), row (bar-code), or map panel (small multiples) toggles focus and re-renders that view with the focused author emphasised and the rest dimmed, and switching to a Topology view recomputes that author's own top-15 places and highlights the matching nodes/edges gold within the current selection's combined graph, dimming the rest. That gives the planned behaviour (``selecting an author in fingerprint view highlights their places in topology view'') without needing \texttt{postMessage} between iframes at all, since the five views now share one document rather than being embedded separately.

The metro map remains the one exception. Because its lines depend on offline community detection (Section~\ref{sec:metro-redesign}) rather than a computation that can run per keystroke in the browser, it stays an \texttt{<iframe>} that switches between four precomputed snapshots (2, 5, 20, and 50 authors, chosen by nearest log-distance match to the current selection) rather than recomputing live, and it does not participate in the focus highlight or the detail panel below. An earlier version of this switch only had two snapshots and picked between them with a fixed threshold, so selecting 2 versus 5 authors, or 20 versus 50, produced no visible change at all. Three snapshots have since been added, at 2 and 50 authors (96\% and 73\% adjacency-backed respectively, extending the monotonic trend already reported for 5 and 20), so every preset now shows a genuinely different, correctly rendered map. Past roughly 20 authors, though, the snapshot is really showing community detection degrading rather than offering a usable exploration surface, so both the standalone scale-exploration tool and the combined interface display a prominent warning at that scale rather than a quiet note, to make clear this is a reported finding and not a rendering fault.

A unified outer detail panel, shared across the five live views, is also implemented, backed by a fifth dataset, \texttt{all\_authors\_sentences.json}. This is generated by a new query against \texttt{api\_sentence} that mirrors the query already used for the five-author prototypes (same sampling, same 220-character truncation, same random seed), but drops the author filter and restricts each of the 408 authors to their own top-15 places by mention count, exactly the set reachable through the live selector, so no sentence data ends up generated for places a user could never hover over. Hovering a place in bar-code, small multiples, network, or linear looks up every author in the current selection against this dataset and merges their example sentences and mention-weighted book list for that place; hovering an author's shape in the radar chart shows their single most-mentioned place as a representative example instead, since radar's own vertices are sectors rather than individual places. The panel is deliberately sticky, not clearing on mouse-out, so a sentence stays readable after the cursor moves away; it does clear when the active visualisation or the author selection changes, since otherwise a stale sentence could reference a place or author no longer in view.

Selecting a large author subset in network or linear turned out to produce enough co-occurrence edges (12,833 at the 50-author preset) that d3's force simulation failed to settle within several seconds, leaving the interface visibly unresponsive. The weight-threshold slider compounded this: it was capped at a fixed maximum of 10 regardless of the true maximum edge weight in the current selection (which reaches into the 80s at scale), so it could not be used to filter down far enough to recover. Both views now cap rendering to the 1,500 strongest-weighted edges, with the slider's maximum computed dynamically from the current graph, and the underlying author selection is left untouched by the cap so switching to another view still reflects the full selection. A related legibility issue turned up in the radar chart's per-author colour assignment: a fixed-step hue formula placed two of fifty authors' colours one degree apart on the colour wheel, indistinguishable despite sitting on the same chart. Hues are now spaced evenly across the current selection size instead, guaranteeing a minimum perceptual gap between every pair shown.

A supervision meeting reviewing the interface at this stage produced three further revisions.

First, the interface had accumulated implementation-facing language visible to anyone using it: the top banner named \texttt{api\_sentence} and internal rendering limits, and small badges read ``live''/``implemented'' next to section headings. None of that helps someone exploring the data decide what to do next, so it was rewritten throughout in plain terms (the metro map's note, for example, now reads that it is ``pre-drawn rather than redrawn live'' instead of describing offline community detection).

Second, the radar chart's hover interaction turned out to offer only one tooltip per author's entire polygon, with no way to inspect a single sector. A small marker was added at each of the 14 sector vertices; hovering one now shows that author's mention count and percentage for that specific sector, together with the individual place names contributing to it (cross-referenced from the bar-code dataset), and updates the outer detail panel to that sector's leading place. The small multiples map markers were extended the same way, gaining sector and percentage-of-total alongside their existing place and mention count.

Third, author selection had previously offered only fixed presets plus one-at-a-time typed search, which does not make ``pick any combination of the 408 authors'' obvious as something you can even do. A collapsible, scrollable checklist of all 408 authors, sorted by mention count, was added alongside the existing controls, and a one-line note now clarifies that the ``2''/``5'' presets are the project's fixed core-author set while ``Top 20''/``Top 50'' means ranked by total corpus mentions, rather than leaving ``top'' unexplained.

The metro map's own further redesign was also discussed in this meeting; the supervisor's response to the current version was positive, and any next iteration is deferred pending a separate follow-up discussion rather than being scoped here.

A fifth author preset, loading all 408 located authors at once, was added alongside the existing 2/5/20/50 presets and explicitly labelled ``large-scale exploration'' in both its button text and an accompanying note. This distinguishes it from the four practical presets: it is meant for spotting broad, corpus-wide patterns rather than reading individual detail, and it is the rendering caps already described above for network, linear, small multiples, and metro that keep it usable at this scale rather than anything that needed disabling.
...
\textbf{Introduction protocol.} Participants are shown each visualisation with only a brief description of the project's purpose (``We are studying how literary place names can be visualised''), but no explanation of what each visual encoding means or how to read it. This ``cold'' presentation matters methodologically, since it tests whether the designs are self-explanatory rather than testing participants' ability to apply an explanation they were given: a participant who needs the design explained before they can use it is, in effect, reporting a usability problem. This is distinct from the labelled reference view used in Task~1 below, which withholds no encoding explanation either; it simply gives participants something to look at before being tested, rather than telling them what to look for.
 
\textbf{Tasks.} Two task types are used. Task~1 (fingerprint direction) tests the success criterion for RQ1 set out in Chapter~\ref{chap:intro}: whether the five authors' fingerprint shapes are distinct enough to be learned and then recognised, not whether a participant can name an author from memory of their books with no visual reference at all -- the latter would conflate visual distinctiveness with prior literary knowledge and would be close to impossible for a participant with no background in these five authors specifically, regardless of how good the encoding is. Task~1 is accordingly a two-step study-then-identify task. Participants first see a labelled reference view showing all five authors' fingerprints together for the design in question (the same canonical, all-five-authors prototype described in Chapter~\ref{chap:design}), and are asked to spend a few minutes noticing how the shapes differ, without being told what to look for. They then see a purpose-built single-author variant of the same design, showing one author's shape only, with the legend and the click-through detail panel (which would otherwise expose book titles and could reveal identity through bibliographic or authorial-voice recognition) removed entirely, leaving only the shape and its hover-revealed sector values, and are asked which of the five authors they now believe it represents, with the reference view still available to revisit. The single-author variant is rendered in a neutral colour rather than that author's colour from the reference view; an earlier version of this stimulus reused the same per-author colour in both views, which would have let a participant answer correctly from a remembered colour swatch alone, without the shape itself playing any role -- testing colour recall rather than the pattern-learnability this task is meant to measure. Removing the colour cue does not by itself rule out a related failure mode: a participant could still answer correctly by matching the shape's outline against a memorised image from the reference view, without engaging at all with what that outline actually communicates -- which geographic sector or place the author's writing concentrates on. Task~1 accordingly opens with an objective, answer-keyed question that can only be answered from the blind stimulus itself, with no memory of the reference view required, asking which sector or place the shape indicates the author writes about most; a participant who cannot answer this correctly has not read the encoding, regardless of whether they go on to guess the right author from shape recognition alone. Accuracy on this identification is the primary metric, but the open-text question asking what specific feature of the pattern led to that answer is treated as equally important, since it captures whether participants can articulate a distinguishing feature even when the forced-choice answer is wrong. A different author is used for each of the three designs, so that a participant's answer on the first fingerprint task does not anchor their answer on the second and third. A single blind identification, however, only ever yields a correct/incorrect judgement against a one-in-five baseline, which says little about which specific authors' fingerprints are most easily confused with one another. Task~1 therefore adds a third step: participants see all five authors' shapes again, unlabelled and in a shuffled order, and match each shape to an author, producing a full $5\times5$ confusion matrix per design rather than a single accuracy figure, plus a self-rated judgement of how easily the five patterns could be confused overall. Both task directions also include items added specifically to guard against ceiling and floor effects on the primary forced-choice and free-text questions: Task~1 adds a runner-up guess after the initial identification and a two-alternative forced choice between the single most confusable pair of authors for each design, identified in advance by cosine similarity on the underlying data; Task~2 adds a rating of three place pairs of known strong/medium/weak co-occurrence weight and a forced choice between a genuine data-derived cluster and two constructed distractors. Full wording and the data-driven basis for each choice are given in Appendix~\ref{app:questionnaire}. Task~2 (topology direction): participants are shown a topology visualisation (force-directed network, linear connection diagram, or metro map) and asked to identify the two places that appear most strongly connected, and to judge whether those two places are geographically close or far apart in Edinburgh. These three designs already aggregate all five authors' co-occurrence data into one structure with every place labelled throughout, so there is no identity to guess and no equivalent reference-then-test structure is needed. The secondary question tests whether the visualisation spontaneously communicates the narrative/geographic divergence finding. After completing all tasks, participants vote for the most intuitive design in each direction.
...
Sixteen responses were collected; three were excluded before analysis on data-quality grounds (Section~\ref{sec:study-procedure}), leaving $n=13$. Participants were recruited through the researcher's personal and academic network within the UK, using the recruitment channels and eligibility criteria set out in the approved ethics application; recruitment was not restricted to the University of Edinburgh.

Grouping into ``domain expert'' and ``general public'' was based on participants' self-described background (Q-B3) rather than assigned at recruitment, following the design set out in Section~\ref{sec:study-design}. Four participants reported a background in literary studies/English literature, digital humanities, or information visualisation/HCI/computer science and were treated as the expert group; the remaining nine, spanning other academic backgrounds and no specialist background, formed the general-public group. This four-to-nine split is thinner on the expert side than the five-to-eight target set during recruitment planning, and is treated throughout this chapter as an exploratory rather than a statistically powered comparison.

A striking feature of the sample, expert group included, is near-uniform unfamiliarity with the five specific authors under study: nine of the thirteen participants had read none of Alexander McCall Smith, Irvine Welsh, John Gibson Lockhart, Walter Scott, or Robert Louis Stevenson, and eight rated themselves ``not at all familiar'' with Edinburgh's literary history. Even the one participant with a literary-studies background reported being only ``slightly familiar'' and had read none of the five authors. This is not a weakness of recruitment so much as a confirmation that the identification tasks in Section~\ref{sec:fp-results} are testing what they are meant to test: whether the visual encoding itself is learnable, rather than whether a participant already happened to know these particular authors. General familiarity with reading data visualisations was more evenly spread, from ``slightly familiar'' to ``extremely familiar,'' with ``moderately familiar'' the most common response; four participants reported having lived in Edinburgh, one had visited once or twice, and eight had never visited.

\section{Study Procedure}
\label{sec:study-procedure}

The study was distributed as an anonymous Qualtrics link and completed online, unmoderated, and at each participant's own pace, following the design set out in Section~\ref{sec:study-design}. Median completion time among the thirteen retained responses was 46 minutes, in line with the estimated 50--65 minutes; the recorded range was wider than this because Qualtrics' ``finish later'' option, enabled given the survey's length, let a handful of participants leave the survey open across more than one sitting, which inflates elapsed time without reflecting active engagement.

Three of the sixteen raw responses were excluded before analysis. One was completed in under ten minutes with near-empty free-text answers (e.g. a single full stop in place of an explanation); a second gave single-letter or otherwise non-committal answers (``a'', ``b'', ``no'') on every free-text field, including ones the anti-memorisation questions (F0, T1b) specifically ask participants to justify in their own words; a third was completed in six minutes with free-text answers unrelated to the questions asked. All three were judged, on this basis, not to reflect genuine engagement with the task, and were removed prior to any quantitative analysis; they are not included in the $n=13$ reported throughout this chapter. A small number of free-text answers in the retained responses were originally given in Chinese by participants who completed the study while resident in the UK; these are translated by the researcher and reproduced alongside the original wording in the underlying response data, rather than replacing it.
...
The clearest single result in this chapter is that the radar chart outperforms the other two fingerprint designs on every measure computed: the highest F0 comprehension rate, the highest single-identification and Top-2 accuracy, the highest five-way matching accuracy, and the highest hardest-pair discrimination. This is consistent with the qualitative pattern in participants' F3 answers, where several radar-chart responses named a specific sector by name (``a very large spike in Leith area'', ``the maximum is old town''), whereas bar-code and small-multiples answers more often described a feature without naming what it corresponded to (``peak value'', ``an unusually large spike''). The polygon's asymmetry appears to give participants a concrete verbal handle on the pattern that the other two encodings, for this sample, did not as reliably provide. Given that F0 accuracy tracks F1 accuracy design-for-design (radar highest on both, small multiples lowest on both), this also supports treating F0 as a genuine check on whether participants engaged with the encoding's content rather than a design's overall shape memorability alone, as intended in Section~\ref{sec:study-design}.

Small multiples' weak performance across F0, F1, and M1 is notable given that, of the three fingerprint designs, it is the one whose reference view is the most visually distinctive at a glance (five separate maps, rather than five overlaid polygons or bar rows): distinctiveness at the reference-viewing stage evidently did not translate into distinguishability once the name was removed and the shape shown in isolation, which is itself informative about what makes a fingerprint encoding learnable, independent of how it happens to look side by side. All three designs nonetheless answered the P1 hardest-pair check well above the 50\% chance baseline (77--85\%), which suggests the underlying data supports pairwise distinguishability even where overall matching accuracy is weaker; the gap between M1 and P1 for small multiples in particular (49\% vs 77\%) indicates that the design's difficulty lies more in distinguishing all five patterns simultaneously than in any single pairwise comparison.

For the topology direction, the network and linear diagram matched each other on the single strongest-pair task (69\% each) but diverged sharply on cluster recognition, with the linear diagram's 23\% barely above what would be expected if participants selected among the three options at random (33\%) and well below the network's 69\%. Read together with the network's markedly stronger strength-rating correlation ($r=0.64$ against $r=0.30$), this points to the same underlying difference: encoding connection strength as edge thickness and spatial proximity (the network) communicates both a single strongest pair and a broader multi-place cluster more reliably than encoding it as arc height along a single axis (the linear diagram), even though the two designs are statistically indistinguishable on the narrower single-pair task alone.

The metro map's result is the most interesting tension in the results. It performed worst of the three topology designs on the specific strongest-pair task (15\%, against 69\% for the other two) despite encoding exactly the same underlying weight as its line structure, yet it was decisively the most popular in the post-task ranking and the most frequently named ``most memorable'' design. A plausible reading, consistent with several participants' own explanations for their ranking choice (``similar to real transit maps I am familiar with'', ``its like a map that we see everyday''), is that the metro map's borrowed visual language is read fluently as a familiar transit map rather than as an instrument for locating one specific strongest edge among many; participants navigate it confidently, but confidence in navigating a representation is not the same as accuracy at the specific task this study asked of it. This is a genuine, if unintended, illustration of the same principle the design chapter draws from Beck's London Underground map (Section~\ref{sec:direction2}): the metro map optimises for a comfortable, generalisable mental model of the network as a whole rather than for precise retrieval of any single edge weight, which is exactly the trade-off the original inspiration for the design makes.

Cross-tabulating by expert/general-public group is reported here with the caveat, stated in Section~\ref{sec:study-design} and repeated because it matters for how much weight to put on it, that four experts is too small a group for a robust quantitative comparison; a single participant's answer shifts the expert-group percentage by 25 points. Descriptively, the expert group's free-text answers were not obviously more accurate on the identification tasks, but were more likely to reference the underlying literary-geographic content directly rather than only the visual encoding, for example one expert participant's cross-design synthesis answer, ``I was surprised that some places that are geographically quite separate still seem closely connected in the authors' writing. It made me realise that literary connections between places do not necessarily follow their physical locations,'' which restates the dissertation's own RQ2 argument in the participant's own words without having been told it in advance. Another expert participant's reflection, ``Yes, Walter Scott in Leith I did not realise this is where his work was set,'' is a small but direct instance of the visualisation surfacing a literary fact the participant did not already know, which is closer to genuine spontaneous discovery than to guided instruction. Both quotes are treated as indicative rather than representative, given the sample size, but they are the kind of qualitative evidence the deep-insight questions (T3, T4, Cross-design synthesis) were designed to surface, and their presence at all, unprompted, in a sample this small is worth weighing alongside the quantitative results above.
...
The results in Section~\ref{sec:fp-results} point to concrete, design-specific changes rather than a single across-the-board fix, since the three fingerprint designs and three topology designs each failed, where they failed, in different ways.

For the small-multiples fingerprint design, the weakest performer on every objective measure, the gap between its own strong reference-view distinctiveness and its weak blind-identification accuracy suggests the problem is not legibility in the abstract but the loss of the side-by-side comparison the reference view provides: participants could tell five maps apart when shown together but struggled with one map in isolation. A design change worth testing is retaining a faint, unlabelled silhouette of the other four authors' spatial envelopes in the blind view, preserving the comparison structure without reintroducing any identifying label. For the bar-code design, the large Top-1-to-Top-2 accuracy gain (46\% to 77\%) suggests participants are extracting real signal (narrowing correctly to a plausible pair) without a reliable way to commit to a single answer; sorting bars by frequency rather than sector by default, rather than requiring participants to switch modes themselves, may make the single largest bar easier to commit to as the deciding feature, which several F3 answers already used as their stated strategy.

For the topology direction, the metro map's weak strongest-pair accuracy despite strong subjective preference is the clearest case for a targeted change: since the design's transit-map familiarity is a genuine strength worth keeping (Section~\ref{sec:fp-results}), the more promising fix is adding a lightweight strength cue within that visual language, such as line thickness or a station-size encoding proportional to interchange weight, rather than asking participants to abandon the metro metaphor for a more data-forward one. The linear diagram's comparatively poor cluster recognition (23\%, barely above the three-option chance baseline) despite matching accuracy on the single-pair task suggests its single-axis arc layout communicates one connection at a time well but not the multi-place groupings a network or metro layout makes visually apparent through spatial adjacency; a targeted improvement would be arc-bundling or colour-coding arcs by community membership, giving the linear diagram an explicit cluster cue it currently lacks.

More generally, the F0 comprehension check's correlation with F1 identification accuracy across all three fingerprint designs (Section~\ref{sec:fp-results}) suggests that future iterations of this evaluation could productively use F0-style objective legibility checks earlier in the design process itself, as a lightweight way to catch a design that looks distinctive but does not actually communicate its content, before committing to a full user study built around it.
 
\section{Narrative Topology vs Geographic Topology}
 
The most striking finding from the data analysis is the co-occurrence of Leith and Princes Street across 28 distinct literary works, the strongest co-occurrence pair in the entire corpus. Geographically the two places sit about 2.5 kilometres apart: Leith a port district historically associated with trade, working-class communities, and political radicalism, Princes Street the principal commercial artery of the Georgian New Town and bourgeois Edinburgh. On a geographic map they are simply two points in the same city. In the narrative topology visualisation they are the closest pair in the entire network.

That is not a coincidence. The journey between Leith and Princes Street recurs as a structuring device in Edinburgh fiction precisely because it crosses a social as well as a spatial boundary. Irvine Welsh's \textit{Trainspotting} makes this explicit, with characters moving between Leith and the city centre as a movement between social worlds, between the margins and the mainstream of Edinburgh life; Walter Scott's historical novels stage similar journeys between the Old Town closes and the wider city as crossings between historical strata. A geographic map cannot show this. The narrative topology visualisation can: it shows these two places as \emph{narratively adjacent}, regardless of their spatial distance.
...
The metro-map redesign (Section~\ref{sec:metro-redesign}) provides a second, independent test of this claim, and a methodological one: if narrative topology genuinely diverges from geography, a line-grouping derived purely from co-occurrence weights should look different from one derived by hand along geographic axes, and should be checkable against the data in a way a hand-drawn grouping is not. The audit bore this out: only 22\% of the hand-drawn map's same-line adjacencies were backed by real co-occurrence, against 88\% once lines were derived from the co-occurrence graph itself, evidence in its own right that geographic intuition is an unreliable guide to narrative structure, consistent with Finding~4 rather than a separate result.

The redesign also surfaced two findings the geographic version could never have produced, since geographic grouping simply cannot detect them. Leith, considered as a place name across all five authors combined, groups with the historical Old Town corridor associated with Scott and Lockhart rather than with the contemporary, working-class Leith of Welsh's novels: at the level of the aggregated five-author corpus, the name's narrative associations pull more strongly toward nineteenth-century historical writing than toward the corpus's single most Leith-saturated author. That is a finding about how a place name functions across a corpus, not about any one author's usage of it, and it only becomes visible once co-occurrence, rather than an author's own usage, is what groups places together. Separately, a five-station corridor (Castle Street, Lasswade, the University of Edinburgh, the Royal Society of Edinburgh, and St Giles) emerged as a coherent cluster corresponding to Lockhart's biographical rather than fictional geography of Scott: these are places Scott lived and worked, as documented in \textit{Life of Walter Scott}, not places his novels are set. The community-detection step distinguishes this biographical register from the historical-fiction register of the neighbouring Old Town corridor without being told to do so.

\section{Author Spatial Languages}
 
The near-total absence of shared place names across the five test authors (zero places shared by three or more, only 18 shared by any two) is a finding that can be stated as a simple number but deserves fuller interpretation. In effect, each author has constructed, across their body of work, an Edinburgh that is almost entirely their own: McCall Smith's is bounded by the triangle formed by Scotland Street, Dundas Street, and Drummond Place in the New Town; Welsh's by Leith; Scott's is centred on the Canongate and the Old Town closes; Stevenson's extends south towards the Pentlands, reflecting his childhood at Swanston Cottage, and west towards the Firth of Forth, reflecting his father's lighthouse engineering work.
...
This vocabulary separation also explains a non-obvious pattern observed when the network graph was scale-tested against the full corpus (\texttt{network\_scale\_explore.html}). Co-occurrence density does not fall smoothly as the number of authors grows: it is high both at the very small end (two authors, or a document set drawn from two or three books) and low at the very large end (all 408 authors), for two different reasons.

At the small end, document-level co-occurrence approaches its degenerate case, since almost any two places mentioned anywhere in the same handful of books will co-occur at least once, a single book's own place vocabulary trivially satisfying ``appears in the same document.'' Restricting the network to three books (two by McCall Smith, one by Welsh) produced 152 places and 5,889 co-occurring pairs at weight $\geq 1$ (51\% of all possible pairs); raising the threshold to weight $\geq 2$ collapsed this to 33 places and 424 pairs, almost all of them connecting McCall Smith's two books to each other rather than to Welsh's. That happens because his fictional Scotland Street recurs across his own works while Welsh's Leith does not recur in either of McCall Smith's, a direct, book-level instance of the vocabulary separation just described.

At the large end, the opposite problem appears: adding authors adds nodes far faster than it adds genuine cross-author edges, for the same underlying reason, so the 408-author graph is large (533 places, 23,380 pairs) but comparatively sparse (16.5\% of possible pairs, against 79.6\% for two authors). The five-author baseline used throughout this dissertation sits between these two failure modes, with enough authors for a cross-author edge to read as a meaningful, comparatively rare signal, rather than either the near-certainty of the two-author or two-book case or the near-absence of the full-corpus case.
...
\textbf{Five-author prototype, tested at scale.} The current system uses five test authors, selected for contrast and representativeness. Rather than leave open whether this scales to the full corpus, it was tested directly for all six designs, using a set of scale-exploration tools kept separate from the six canonical five-author prototypes (radar, bar-code, network, and linear are live interactive pages that recompute their data client-side for any of the 408 authors with location data; small multiples and metro required batch or capped-live testing for reasons specific to each design, detailed below). The headline result is that the six designs do not share one scaling bottleneck. Each fails in a different, specific way, summarised in Table~\ref{tab:scale-summary} (Appendix~\ref{app:scale}).

Radar and bar-code (Direction 1) both degrade by losing the ``compare everything at a glance'' property, but through different mechanics. Radar's static polygon overlay becomes visually unreadable well before 408 authors, with individual shapes no longer distinguishable by 20, though a hover-to-isolate interaction recovers single-author legibility at any $N$ at the cost of the all-at-once view. Bar-code never becomes unreadable or slow (408 rows render in 20 milliseconds), but the page ends up roughly 50 screens tall, so comparing two arbitrary authors means scrolling between points far apart rather than looking at two adjacent shapes.

Network and linear (Direction 2) share a root cause, one mark per co-occurring place pair laid out along a single spatial dimension with no wrapping, but it shows up differently in each. Network's force-directed layout degrades gracefully: only the graph's dense core becomes unreadable, and the periphery stays legible even at 408 authors. Linear's fixed horizontal axis instead just grows the canvas to tens of thousands of pixels wide, so a screenshot of the visible, scrolled viewport looks almost unchanged from the five-author version even though the underlying canvas is roughly 27 screen-widths across.

Small multiples is the only design with a measured, genuine computational cost rather than a purely visual or navigational one, since each panel is a live map instance requesting real tiles. Initialisation time per panel stays roughly constant from 5 to 50 authors ($\approx$1--3ms each) but degrades by a factor of 3.6 from 50 to 408 (measured directly, not extrapolated, at 1.47 seconds total): a real main-thread delay a user would feel as a freeze, and distinct from the other designs' purely visual degradation.

Metro is the only design where the failure sits in the underlying algorithm rather than the picture. Re-running the community-detection pipeline (Section~\ref{sec:metro-redesign}) at larger $N$ shows the adjacency-backed percentage declining monotonically (96\% at 2 authors, 92\% at 5, 84\% at 20, 72\% at 50), and at 50 authors one community grows to 75 stations because \texttt{greedy\_modularity\_communities} judges it internally cohesive enough that the recursive size cap cannot split it further. That is a structural limit of the clustering approach, not a rendering or layout problem.
 
\textbf{Metro-map layout is heuristic, not optimal.} The metro-map redesign (Section~\ref{sec:metro-redesign}) replaces manual line assignment with community detection and manual station placement with a spring embedding corrected for octilinearity. Neither step solves the underlying schematic-layout problem exactly, since N{\"o}llenburg and Wolff's result \cite{nollenburg2006} establishes that an exact solution is computationally hard in general, and two residual limitations follow directly from this.

First, the line structure is sensitive to the community-detection threshold and the five-author, top-15-places-per-author filter used to build the co-occurrence graph. This was confirmed directly rather than left as speculation (Appendix~\ref{app:scale}): re-running the pipeline at 20 and 50 authors produces 8 and 11 lines respectively, with the proportion of adjacency-backed adjacent stations falling from 92\% at 5 authors to 72\% at 50, and at 50 authors one community grows to 75 stations because \texttt{greedy\_modularity\_communities} treats it as internally unsplittable despite exceeding the 18-station recursion cap. That is a structural limit of the clustering approach; the current five-line map avoids it only because it happens to sit at a favourable point on this curve, not because the underlying method guarantees it, so the current five lines should not be read as a fixed taxonomy. Second, the greedy label-placement search leaves two of 57 station labels overlapping in the single densest interchange cluster, which a fully general solver (or manual adjustment, common practice even for professionally drawn transit maps) could resolve.
...
\textbf{User study sample size.} The evaluation reported in Chapter~\ref{chap:eval} draws on $n=13$ participants, below the 13--20 planned during recruitment and well below what would support statistically powered comparisons; percentages throughout Chapter~\ref{chap:eval} should be read as directional rather than precise estimates, with a single participant's answer worth roughly 8 percentage points on any given measure. The domain-expert subgroup ($n=4$) is thinner still, and the expert/general-public comparisons in Section~\ref{sec:fp-results} are accordingly treated as indicative rather than conclusive throughout. Recruitment was also convenience-based, through the researcher's own personal and academic network rather than a broader public call, which likely skews the sample toward participants already favourably disposed to engage carefully with the task; this is a plausible contributor to the generally low rates of self-reported prior confusion with the visualisations (Q-R3), and a larger, more broadly recruited sample would be needed before drawing firm conclusions about how well these designs generalise beyond a personal network. The exclusion of three low-quality responses (Section~\ref{sec:study-procedure}) was based on researcher judgement against criteria (completion time, free-text engagement) that were not formally pre-registered before data collection began, which is a standard limitation of a small, time-constrained study rather than one specific to this project, but is stated here for transparency.
...
\textbf{RQ2 --- does narrative co-occurrence reveal a topology distinct from geography?} Yes. The strongest evidence is the Leith/Princes Street pair itself: 28 shared works despite roughly 2.5km of geographic separation, and one of only 12 station-pairs across the entire redesigned metro map that behave as a genuine narrative interchange. The redesign of the metro-style map (Section~\ref{sec:metro-redesign}) turned this from a single anecdote into a systematic result, deriving line structure from measured co-occurrence rather than hand-drawn geography and raising the proportion of adjacent stations sharing a real book from 22\% to 88\%, while surfacing two further instances of the same divergence that geography could not have produced: the word ``Leith'' clustering with Scott and Lockhart's historical corridor rather than Welsh's, and a biographical corridor (Castle Street, Lasswade, the University of Edinburgh) distinct from the historical-fiction corridor immediately next to it. The scale-exploration work (Appendix~\ref{app:scale}) adds a boundary condition here rather than a contradiction: narrative and geographic topology diverge clearly at the five-author scale used throughout this dissertation, but the mechanism generating that divergence, near-total non-overlap of authorial vocabularies (Finding~1), also means the co-occurrence signal thins out at very large author counts and degenerates at very small ones (2--3 books). The divergence is most legible at a middle scale, in other words, rather than a universal property independent of $N$.

\textbf{RQ1 --- does an author's spatial distribution form a visually distinctive, learnable fingerprint?} Partially yes. Across the user study ($n=13$), the radar chart supported the strongest evidence for this claim: after studying all five authors' fingerprints together, participants correctly matched all five unlabelled shapes back to their authors at 69\% accuracy, well above the 20\% chance baseline, and correctly distinguished the single most similar pair of authors 85\% of the time. The bar-code and small-multiples designs showed the same direction of effect but more weakly, with matching accuracy close to half (48\% and 49\% respectively) rather than clearly distinguishable, and single-shape identification in isolation (23--46\% across the three designs) was consistently harder than matching all five side by side, indicating that these fingerprints are more reliably told apart from one another than recognised from memory alone. The evidence therefore supports RQ1 for at least one encoding, the radar chart, without supporting a stronger claim that spatial fingerprinting works equally well regardless of how the fingerprint is drawn; which specific encoding choices explain the radar chart's advantage, and whether it would hold at a larger sample size, is discussed in Chapter~\ref{chap:discussion}.
...
Several directions for future work follow from where the project currently stands. The combined interface (Section~\ref{sec:combined-interface}) already provides author-selection controls reaching the full 408-author corpus for five of the six views, with rendering safety caps keeping it responsive at scale, but the metro map's dependence on offline community detection is still a gap: it currently limits the map to a small number of precomputed author-count snapshots rather than an arbitrary live selection, something a faster, approximate community-detection method running client-side could close. The interactive detail panels are likewise already grounded in \texttt{api\_sentence} source text (Section~\ref{sec:combined-interface}), and a natural extension would be a single outer panel shared consistently across all six canonical views, rather than each view's own panel plus the combined interface's separate one. Beyond the interface itself, the \texttt{api\_posmention} table's two million-plus part-of-speech annotations would support a further research direction in narrative weight analysis: whether a place name appears in dialogue, narration, or description, and whether this varies systematically across authors or genres. And the concept of ``literary silences,'' Edinburgh places that appear in the gazetteer but never in any literary work, would complement the current analysis by revealing what Edinburgh's literary imagination systematically excludes.
...
The following instrument implements the two task types described in the Study Design section of Chapter~\ref{chap:eval}. Each participant sees all six visualisations; the order of the three fingerprint designs (and, separately, the three topology designs) is randomised per participant to control for ordering effects. Wording is final; formatting into the online survey tool is the only remaining step before the study can run.
