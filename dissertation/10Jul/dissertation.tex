...
The metro map remains the one exception: because its lines depend on offline community detection (Section~\ref{sec:metro-redesign}) rather than a computation that can run per keystroke in the browser, it stays an \texttt{<iframe>} that switches between the two precomputed snapshots already produced for the scale exploration (5-author canonical, 20-author) based on how many authors are currently selected, and does not participate in the focus highlight. This is reported honestly in the interface itself rather than silently failing to update. The one feature still not built is a unified outer detail panel driven by \texttt{api\_sentence} lookups shared across views; each view already exposes its own hover/click detail (tooltips with mention counts, co-occurrence weights, and sector breakdowns), so this is a polish item rather than a functional gap.
