...
<div class="status-banner">
  ...
  between four precomputed snapshots (2/5/20/50 authors, nearest match) rather than recomputing live, and does not feed the detail panel. Past ~20 authors the snapshot itself demonstrates community detection breaking down &mdash; that is a real finding, not a rendering bug.
...
function renderMetro() {
  ...
  const degradeWarning = closest.n >= 20
    ? ` At this scale the map demonstrates community detection breaking down (see Appendix), not a legible exploration surface -- use the 2 or 5 preset to see the design at its intended scale.`
    : '';
  note.textContent = `${selectedAuthors.length} author(s) selected — metro lines cannot be recomputed live (community detection runs offline), so this shows the closest precomputed snapshot: ${closest.n} authors${exact ? ' (exact match)' : ''}.${degradeWarning}`;
  note.classList.toggle('severe', closest.n >= 20);
  setStatus(`Metro map: precomputed ${closest.n}-author snapshot, independent of the shared author selector above.`);
}
