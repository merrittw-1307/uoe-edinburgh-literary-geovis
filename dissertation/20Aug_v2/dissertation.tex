...
\begin{enumerate}
  \item \textbf{Community detection.} Modularity-based community detection (\texttt{networkx.community.greedy\_modularity\_communities}) is applied to the 57-place, 403-pair co-occurrence graph (edges with weight $\geq 3$), so line membership reflects measured narrative connectivity rather than manual geographic grouping. Communities larger than 18 stations are recursively re-clustered on their own subgraph; unconnected places are folded into whichever cluster they connect to most strongly in the unthresholded graph.
  \item \textbf{Station ordering.} Within each line, stations are ordered by a nearest-neighbour chain over real edge weights, starting from the strongest internal pair, so adjacency on a line is backed by measured co-occurrence wherever the graph permits it.
  \item \textbf{Interchanges.} A pair of stations on different lines with co-occurrence weight $\geq 10$ becomes a genuine interchange (a real stop on the second line, not an illustrative cross-reference); this threshold was chosen empirically to yield a legible 12 of 57 stations as interchanges (a lower threshold produced 28, too dense to read as meaningful hubs).
  \item \textbf{Line naming.} Assigned automatically: a line is named after a single author if they account for at least 55\% of its home stations' book-mention volume (e.g. ``Welsh's Edinburgh''), otherwise after its two highest-mention stations (e.g. ``Leith \& Forth Corridor'').
  \item \textbf{Layout.} A shared coordinate for every station is computed via a co-occurrence-weighted spring embedding (\texttt{networkx.spring\_layout}), so lines sharing an interchange meet at one physical point rather than being drawn in separate lanes. Positions are snapped to an integer grid with collision avoidance; the one station with zero measured co-occurrence is anchored to its line's centroid.
  \item \textbf{Octilinear correction and labels.} Line segments not already aligned to Beck's angle convention ($0^\circ$/$45^\circ$/$90^\circ$) are redrawn as a two-part ``dog-leg'' rather than an arbitrary diagonal. Station labels are placed by a greedy search over eight compass positions, choosing the first that does not overlap an already-placed label or marker.
\end{enumerate}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.68\textwidth]{figures/fig_metro_v1_geographic.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.68\textwidth]{figures/fig_metro.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.68\textwidth]{figures/fig_radar.png}
...
Hover interaction uses invisible \texttt{circle} targets (radius 14px, chosen for comfortable targeting) centred on each vertex. On hover, a tooltip shows the sector name and percentage; on click, a detail panel opens showing the top eight place names in that sector for the selected author, ordered by mention count, together with a bar chart and the top five books.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.68\textwidth]{figures/fig_barcode.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.68\textwidth]{figures/fig_small_multiples.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.68\textwidth]{figures/fig_network.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.68\textwidth]{figures/fig_linear.png}
...
\textbf{Versioning and compatibility.} Each HTML file is versioned under filenames such as \texttt{radar\_v1.html}, preserving the design evolution as a record for the dissertation and the supervisor. All files were tested in Safari and Chrome on macOS; D3.js v7 and Leaflet.js 1.9.4 are loaded from CDN at runtime, requiring an internet connection.
