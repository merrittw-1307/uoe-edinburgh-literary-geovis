...
N{\"o}llenburg and Wolff's work on algorithmic metro-map layout \cite{nollenburg2006} is relevant to the metro-style visualisation as a precedent and as a justification for the decision to use manual rather than algorithmic layout: schematic metro-map drawing, which requires placing nodes on an octilinear grid while preserving topological correctness and legibility, is a known computationally hard problem. The manually constructed metro map in this project is therefore an intentional simplification, appropriate for prototyping but not scalable to the full author corpus.
...
The primary interaction is point-level: hovering over any vertex reveals the sector percentage and a ranked list of the places and books that contribute to it. This responds directly to the supervisor's feedback that the visualisation should provide ``low-level information upon interaction.'' A detail panel opens on click, listing the top place names and top books in the selected sector for the selected author. The legend supports filtering by author (click to hide/show) and isolation (double-click to show only one author), enabling the user to compare any pair or subset of authors.
...
\textbf{Metro-style map (illustrative design).} The metro-style map arranges place names as stations on eight named lines, following the visual language of the London Underground map. The London Tube map, designed by Harry Beck in 1933, replaced geographic accuracy with functional topology: it told travellers which line to take and where to change, rather than where stations were physically located. This project applies the same substitution to literary geography: the lines represent narrative corridors (clusters of places that appear together in a thematic or geographic tradition), not physical routes.
 
Eight lines are defined, each named for a geographic or cultural axis: North Line (coastal north Edinburgh, from Granton to Cramond), Old Town Line (the Royal Mile from Grassmarket to Duddingston), South Line (the southern residential belt from Colinton to Musselburgh), East-West Line (cross-city from Corstorphine to Portobello), Circle Line (the inner-city loop through Tollcross, Bruntsfield, and back via the Old and New Towns), North-South Line (a meridional axis from Leith to Fairmilehead), Outer Line (suburban and satellite towns), and West Line (the western approach via Cramond and Murrayfield). Interchange stations---served by two or more lines---are marked with a double-circle symbol.
 
The layout is manually constructed on a regular grid (unit spacing 58 pixels) rather than generated algorithmically. This decision reflects the state of the art in metro-map drawing: algorithmic schematic layout, which requires placing nodes at octilinear angles while preserving topological correctness, is a known computationally hard problem \cite{nollenburg2006}. A manually constructed map is appropriate for the five-author prototype; automated layout would be required for a system scaled to the full 424-author corpus.
...
\midrule
1 & Radar chart & Polygon shape across 14 sector axes & Click vertex for sector detail panel \\
1 & Bar-code & Bar height = mention share per place & Sector/frequency/alpha ordering; per-author Y-axis \\
1 & Small multiples & Bubble size on real map & Sync zoom; log/linear toggle \\
2 & Force-directed network & Node size = frequency; edge width = co-occurrence & Weight slider; drag nodes \\
2 & Linear connection & Arc height = node distance; arc width = weight & Weight slider; ordering \\
2 & Metro-style map & Line membership = narrative corridor & Zoom/pan; hover for line info \\
\bottomrule
\end{tabular}
\end{table}
...
The map is rendered as a static SVG with eight polyline paths (one per line), \texttt{circleMarker} elements for stations, and text labels. Interchange stations (those served by two or more lines) are marked with an outer white circle of radius 10 and an inner dark circle of radius 4, following the London Underground convention. All other stations use a smaller white circle with a coloured stroke matching the line colour. D3 zoom behaviour is attached to the SVG to support scroll-to-zoom and drag-to-pan.
...
\textbf{Manual metro-map layout.} The metro-map layout is manually constructed and is not generalisable to arbitrary data. A system that assigned stations to lines algorithmically, and laid the map out on an octilinear grid, would be required to scale this design to the full corpus or to allow users to define their own line groupings.
...
\textbf{Source text integration.} The \texttt{api\_sentence} table contains the full source sentence for every mention, but this data has not yet been integrated into the interactive visualisations. The supervisor identified this as a priority: being able to read the actual literary sentence in which a place appears is the most direct form of ``details on demand'' the data supports. This feature is planned for the combined interface.
